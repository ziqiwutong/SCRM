package com.scrm.manage.exception;
...
//
//        // 打印堆栈，以供调试，上线之前需要注释掉
//        System.out.println("sa-token全局异常处理，上线之前需要注释掉打印信息---------------");
//        e.printStackTrace();
//
//        // 不同异常返回不同状态码
//        Result result = null;
...
//    }
    ...
    public Result handle(IOException ie) {
        if (ie instanceof FileSizeLimitExceededException) {
            return Result.error(CodeEum.PARAM_ERROR, "上传文件过大，最大5MB");
        }

        return Result.error(CodeEum.ERROR, "IO ERROR: " + ie.getMessage());
    }
